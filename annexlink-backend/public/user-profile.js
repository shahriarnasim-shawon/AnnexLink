...
function renderReviews(reviews) {
    ...
    reviews.forEach(review => {
        ...
        const rAvatar = review.reviewer ? (review.reviewer.avatar.startsWith('http') ? review.reviewer.avatar : `${review.reviewer.avatar}`) : `https://ui-avatars.com/api/?name=U`;
...
document.getElementById('review-form').addEventListener('submit', async (e) => {
    ...
    try {
        const response = await fetch(`/api/users/${targetUserId}/reviews`, {
            method: 'POST',
            headers: {
...
async function submitReport() {
    ...
    try {
        const response = await fetch(`/api/users/${targetUserId}/report`, {
            method: 'POST',
            headers: { 
                'Content-Type': 'application/json',
                'Authorization': `Bearer ${token}` 
...
function renderPublicPosts(posts) {
    ...
    posts.forEach(post => {
        ...
        if (post.media) {
            ...
            const mediaUrl = cleanPath.startsWith('/') ? `${cleanPath}` : `/${cleanPath}`;
